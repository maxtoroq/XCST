...
<stylesheet version="2.0" exclude-result-prefixes="#all"
   ...
   <template match="c:member" mode="src:member">
      ...
      <call-template name="xcst:validate-attribs">
         <with-param name="required" select="'name'"/>
         <with-param name="optional" select="'as', 'value', 'expression', 'auto-initialize', 'display', 'display-name', 'description', 'short-name', 'edit-hint', 'order', 'group', 'format', 'apply-format-in-edit-mode', 'disable-output-escaping', 'null-display-text', 'template', 'read-only', 'data-type', 'required', 'max-length', 'min-length', 'pattern', 'min', 'max', 'equal-to', $xcst:type-or-member-attributes, $xcst:validation-or-member-attributes"/>
      </call-template>
      <call-template name="xcst:validate-children">
         <with-param name="allowed" select="'metadata', 'member'"/>
      </call-template>
